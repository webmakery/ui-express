...
class uipress_compiler
{
  ...
  public function run()
  {
    ...
    // Safety fallback for installs where activation hooks did not fire correctly.
    add_action("admin_init", [$this, "maybe_add_required_caps"]);
  }


  /**
   * Ensures required caps exist even if activation hooks were skipped.
   *
   * @since 3.3.02-patched
   */
  public function maybe_add_required_caps()
  {
    ...
    if (!$role->has_cap("uip_manage_ui") || !$role->has_cap("uip_delete_ui")) {
      $this->add_required_caps();
    }
  }
  ...
  {
    register_activation_hook(uip_plugin_file, [$this, "add_required_caps"]);
    register_deactivation_hook(uip_plugin_file, [$this, "remove_required_caps"]);
  }
